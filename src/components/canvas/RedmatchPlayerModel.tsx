...
export const RedmatchPlayerModel: React.FC<RedmatchPlayerModelProps> = ({ player }) => {
  ...
  // Offset camera eye height (1.62m) so feet are firmly planted on the ground (Y = 0.00m)
  ...
  useFrame((_, delta) => {
    ...
    // High-performance smooth position and rotation lerping
    groupRef.current.position.lerp(targetPos.current, Math.min(1, delta * 28));
    groupRef.current.rotation.y = THREE.MathUtils.lerp(groupRef.current.rotation.y, targetYaw.current, Math.min(1, delta * 24));

    // Pitch upper body and weapon with aim direction
    if (upperBodyRef.current) {
      upperBodyRef.current.rotation.x = THREE.MathUtils.lerp(upperBodyRef.current.rotation.x, targetPitch.current, Math.min(1, delta * 24));
    }
    ...
    if (speed > 0.4 && leftLegRef.current && rightLegRef.current) {
      ...
      leftLegRef.current.rotation.x = THREE.MathUtils.lerp(leftLegRef.current.rotation.x, 0, delta * 14);
      rightLegRef.current.rotation.x = THREE.MathUtils.lerp(rightLegRef.current.rotation.x, 0, delta * 14);
    }
  });
    ...
    >
      ...
      <Html position={[0, 2.25, 0]} center distanceFactor={14} zIndexRange={[100, 0]}>
        <div className="flex flex-col items-center pointer-events-none select-none">
          <div className="px-2.5 py-0.5 rounded-lg bg-black/85 border border-white/20 backdrop-blur-md text-[11px] font-black text-white whitespace-nowrap shadow-xl flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full shadow-[0_0_8px_currentColor]" style={{ backgroundColor: color }} />
            <span>{player.nickname}</span>
          </div>

          {/* 100 HP Bar */}
          <div className="w-22 h-2 bg-black/90 rounded-full mt-1 border border-white/20 overflow-hidden shadow-lg p-0.5">
            <div
      ...
      {/* UPPER BODY GROUP (Pitches with player look angle) */}
      <group ref={upperBodyRef} position={[0, 1.0, 0]}>
        {/* Sleek Minimalist Geometric Torso */}
        <mesh
          position={[0, 0, 0]}
          castShadow
        ...
        {/* HEAD & HAT (Y = +0.55 relative to upper body = 1.55m world) */}
        <group position={[0, 0.55, 0]}>
          ...
          {/* Sleek Dark Visor & Glowing Eye Line */}
        ...
        {/* FLOATING HANDS & 3D WEAPON HELD FORWARD */}
        ...
        {/* High-Visibility 3D Weapon Model Held in Hands Pointing Forward */}
        <group position={[0.26, -0.06, 0.38]} rotation={[0, 0, 0]}>
          {renderWeaponModel(weapon)}
        </group>
      </group>

      {/* LOWER BODY: Left & Right Legs */}
      <mesh
        ref={leftLegRef}
      ...
      {/* Full Body Raycast Precision Cylinder */}
      <mesh
        position={[0, 0.95, 0]}
        userData={{ targetId: player.id, isHeadshot: false, isRemotePlayer: true }}
      >
...
function renderGeometricHat(hatType: HatType, color: string, playerId: string) {
  ...
  switch (hatType) {
    ...
    case 'crown':
      return (
        <mesh position={[0, 0.28, 0]} userData={ud}>
          ...
          <meshStandardMaterial color="#fbbf24" roughness={0.2} metalness={0.9} emissive="#fbbf24" emissiveIntensity={0.3} />
        </mesh>
      );
    case 'horns':
      return (
        <group position={[0, 0.24, 0]}>
          <mesh position={[-0.16, 0.14, 0]} rotation={[0, 0, 0.4]} userData={ud}>
            <coneGeometry args={[0.07, 0.26, 8]} />
            <meshStandardMaterial color="#ff0055" emissive="#ff0055" emissiveIntensity={1.0} />
          </mesh>
          <mesh position={[0.16, 0.14, 0]} rotation={[0, 0, -0.4]} userData={ud}>
            <coneGeometry args={[0.07, 0.26, 8]} />
            <meshStandardMaterial color="#ff0055" emissive="#ff0055" emissiveIntensity={1.0} />
    ...
    case 'pyramid':
      return (
        <mesh position={[0, 0.36, 0]} rotation={[0, Math.PI / 4, 0]} userData={ud}>
...
function renderWeaponModel(weapon: 'vandal' | 'sheriff' | 'knife') {
    ...
    return (
      <group rotation={[Math.PI / 2, 0, 0]} scale={0.8} position={[0, 0, -0.15]}>
        ...
        <mesh position={[0, 0.2, 0]}>
          <boxGeometry args={[0.025, 0.4, 0.07]} />
          <meshStandardMaterial color="#00f0ff" emissive="#00f0ff" emissiveIntensity={1.8} roughness={0.1} metalness={0.9} />
        </mesh>
        {/* Handle */}
  ...
  if (weapon === 'sheriff') {
    return (
      <group scale={0.85} position={[0, 0, -0.2]}>
        {/* Heavy Gold/Obsidian Barrel */}
        <mesh position={[0, 0.05, -0.22]}>
          <boxGeometry args={[0.07, 0.12, 0.42]} />
          <meshStandardMaterial color="#fbbf24" metalness={0.9} roughness={0.2} emissive="#fbbf24" emissiveIntensity={0.3} />
        </mesh>
        {/* Grip */}
  ...
  // 3D PRELUDE VANDAL RIFLE (High visibility with glowing barrel & receiver)
  return (
    <group scale={0.75} position={[0, 0, -0.32]}>
      {/* Long Main Barrel & Suppressor */}
      <mesh position={[0, 0.05, -0.35]}>
        <boxGeometry args={[0.07, 0.1, 0.75]} />
        <meshStandardMaterial color="#0284c7" metalness={0.85} roughness={0.25} emissive="#00f0ff" emissiveIntensity={0.8} />
      </mesh>

      {/* Receiver / Body */}
      <mesh position={[0, 0.03, 0.05]}>
        <boxGeometry args={[0.09, 0.14, 0.45]} />
        <meshStandardMaterial color="#0f172a" metalness={0.7} roughness={0.4} />
      </mesh>

      {/* Curved Magazine */}
      <mesh position={[0, -0.15, -0.08]} rotation={[-0.25, 0, 0]}>
