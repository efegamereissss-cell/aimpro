...
import { Heart, Eye, MessageSquare, Play, Pause, Volume2, VolumeX, MapPin, Share2, Trash2 } from 'lucide-react';
...
export const ClipCard: React.FC<ClipCardProps> = ({ clip }) => {
  ...
  const isOwner = clip.ownerId === currentUserId;
  const ytEmbed = getYouTubeEmbedUrl(clip.videoUrl);
  ...
  return (
    <div className="bg-[#0E1322]/90 backdrop-blur-2xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl hover:border-[#FF4655]/40 transition-all flex flex-col justify-between group">
      ...
      <div className="relative aspect-video bg-black overflow-hidden group/video">
        {ytEmbed ? (
          ...
          <div onClick={togglePlay} className="w-full h-full cursor-pointer relative">
            <video
              ref={videoRef}
              src={clip.videoUrl}
              loop
            ...
            <div className="absolute top-3 left-3 right-3 flex items-center justify-between pointer-events-none">
              <div className="flex items-center gap-1.5">
                <span className="px-2.5 py-1 rounded-xl bg-black/80 backdrop-blur-md border border-white/20 text-xs font-black text-white flex items-center gap-1 shadow-md">
                  <span>{clip.agent}</span>
                </span>
                <span className="px-2.5 py-1 rounded-xl bg-black/80 backdrop-blur-md border border-white/20 text-xs font-bold text-white/80 flex items-center gap-1 shadow-md">
              ...
              <button
                onClick={toggleMute}
                className="pointer-events-auto p-1.5 rounded-xl bg-black/80 backdrop-blur-md border border-white/20 text-white/80 hover:text-white transition-colors"
              >
            ...
            <div className="absolute bottom-3 left-3 flex items-center gap-2 pointer-events-none">
              ...
              <span className="px-2.5 py-0.5 rounded-lg bg-black/80 text-white text-[11px] font-bold">
                {clip.authorName}
              </span>
            </div>
          </div>
      ...
      <div className="p-4 space-y-3">
        <div>
          ...
          <p className="text-xs text-white/60 font-medium line-clamp-2 mt-1 leading-relaxed">
            {clip.description}
          </p>
        </div>
        ...
        {clip.tags && clip.tags.length > 0 && (
          <div className="flex items-center gap-1.5 flex-wrap">
            {clip.tags.map((tag, i) => (
              <span key={i} className="text-[10px] font-semibold px-2 py-0.5 rounded-lg bg-white/5 text-white/60 border border-white/5">
                #{tag}
              </span>
            ))}
          </div>
